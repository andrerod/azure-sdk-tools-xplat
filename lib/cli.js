...
_.extend(AzureCli.prototype, {
    ...
    return self.action(function () {
      ...
      try {
        ...
        for (var i = 0; i < arguments.length; ++i) {
          ...
          // Set positional in the options argument
          var argument_options = self.args.filter(function (a) {
            return a.position === (i + 1);
          })[0];

          arguments[optionIndex][argument_options.name] = arguments[i];
        }
        ...
        // TODO: detect the ones that are still missing
  ...
  argument: function(options) {
    if (options.position && (options.position < 1 || options.position > (this.args.length + 1))) {
      throw new Error('Invalid argument position');
    }

    if (options.position) {
      this.args.splice(options.position, 0, {
        required: (options.required && !options.short && !options.long),
        name: options.name,
        actual_required: options.required,
    ...
    if (options.short && options.long) {
      this.option(util.format('%s, %s <%s>', options.short, options.long, options.name), options.description);
    } else if (options.short) {
      this.option(util.format('%s %s', options.short, options.name), options.description);
    } else if (options.long) {
      this.option(util.format('%s %s', options.long, options.name), options.description);
    }
