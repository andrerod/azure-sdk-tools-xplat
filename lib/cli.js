...
function AzureCli(name, parent) {
  ...
  if (!parent) {
    this.complete = omelette('azure <action>');
    ...
    this.complete.on('action', function() {
  ...
  */
  ...
  helpInformation: function() {
    ...
    if (!self.parent) {
      ...
      // TODO: might be missing cli here
      self.helpOptions();
    }
  ...
  helpCategoriesSummary: function() {
    var categories = [];
    ...
    scan(this, function (cat) { categories.push(cat); });
    var maxLength = 14;
  ...
  helpOptions: function() {
    ...
    if (self.parent) {
      self.options = self.parent.options.concat(self.parent.options);
    }
