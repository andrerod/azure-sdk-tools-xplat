  ...
  execute: function (fn) {
    ...
    return self.action(function () {
      ...
      try {
        ...
        var args = new Array(self.args.length + 2);
        ...
        var options = arguments[optionIndex];
        ...
        for(var j = 0; j < self.args.length; j++) {
          // Some commands use, name as a parameter
          // which is also used by commander as the command name
          // this creates a conflict, so we need to ignore that 
          // in the options. No non positional argument can be named name.
          if (options[self.args[j].name] &&
            self.args[j].name !== 'name') {
            args[j] = options[self.args[j].name];
            delete options[self.args[j].name];
