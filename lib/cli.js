...
_.extend(AzureCli.prototype, {
  ...
  argument: function(options) {
    ...
    if (options.position) {
      this.args.splice(options.position, 0, { required: (options.required && !options.short && !options.long), name: options.name, actual_required: options.required });
    }
...
*/
    /*
    var args = flags.split(/ +/);
    console.log(args);
    console.log(AzureCli['super_'].toString());
*/

    // var cmd = new AzureCli(args.shift(), this);

    // console.log(this.command.toString());
    // console.log(this.option.toString());
