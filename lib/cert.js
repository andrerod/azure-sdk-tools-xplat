...
exports.init = function (cli, parent) {
      ...
      if (!options.hostedService) {
        ...
        utils.doServiceManagementOperation(channel, 'listHostedServices', function(error, response) {
          ...
          if (!error) {
            ...
            var forEachService = function (error, response, service) {
              pending--;
              if (pending === 0) {
                progress.end();
              }
  ...
  vm.command('delete <thumbprint>')
    ...
    if (options.dnsName) {
      utils.doServiceManagementOperation(channel, 'deleteCertificate', utils.getDnsPrefix(options.dnsName), 'sha1', thumbprint, function(error) {
        progress.end();
        callback(error);
      });
      return;
    ...
    utils.doServiceManagementOperation(channel, 'listHostedServices', function(error, response) {
      ...
      var deleteCertificate = function (dnsPrefix) {
        var curName = dnsPrefix;
        var caagCallback = caagg.getCallback(dnsPrefix); // insert name
        utils.doServiceManagementOperation(channel, 'deleteCertificate', dnsPrefix, 'sha1', thumbprint, function(error) {
