  ...
  */
  ...
  mobile.command('list')
        ...
        .execute(function (options, _) {
          var progress = cli.interaction.progress($('Getting mobile services'));
          var result;
          try {
  ...
  mobile.command('log [servicename]')
        ...
        .execute(function (servicename, options, _) {
          ...
          var progress = cli.interaction.progress($('Retrieving logs'));
          var result;
          try {
  ...
  mobileTable.command('create [servicename] [tablename]')
        .usage('[options] [servicename] [tablename]')
        .description($('Create a mobile service table'))
  ...
  mobileScript.command('list [servicename]')
        ...
        .execute(function (servicename, options, callback) {
          ...
          function ensuredServiceName(servicename) {
            ...
            var progress = cli.interaction.progress($('Getting script information'));
            var results = {};
            var operationCount = 0;
            function tryFinish() {
              if (++operationCount < 3) {
  ...
  function saveScriptFile(scriptSpec, script, output, force) {
    ...
    if (utils.pathExistsSync(file) && !force) {
      return util.format($('File %s already exists. Use --override to override'), file);
    }
    else {
      try {
        if (!output) {
        ...
        return util.format($('Unable to save file %s'), file);
      }
    }
