  ...
  site.doSitesGet = function (options, _) {
    ...
    try {
      var result = async.map(spaces,
        function (webspace, _) {
          return service.webSpaces.listWebSites(webspace.name, _).webSites;

          // TODO: uncomment when the listWebsites API supports propertiesToInclude
          /*
...
*/
  ...
  site.doSiteGet = function (options, _) {
    ...
    try {
      result = service.webSites.get(options.site.webspace, options.site.name, _).webSite;
    } catch (err) {
