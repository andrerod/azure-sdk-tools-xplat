...
exports.init = function (cli) {
  ...
  function getRepositoryUri(siteData) {
    if (siteData.siteProperties.properties) {
      console.log(siteData.siteProperties.properties);
      for (var i = 0; i < siteData.siteProperties.properties.length; ++i) {
        var pair = siteData.siteProperties.properties[i];
  ...
  site.doSitesPost = function (options, _) {
    ...
    try {
      ...
      console.log(options.site.webspace);
      console.log('couves');
      service.webSites.create(options.site.webspace, options.site, _);
    } finally {
      progress.end();
    }
