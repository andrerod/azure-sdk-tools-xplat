...
exports.init = function (cli) {
  ...
  site.command('list')
        ...
        .execute(function (options, _) {
          ...
          try {
            locations = service.webSpaces.list(_);
  ...
  site.command('set [name]')
        ...
        .option('-w, --web-socket', $('use this flag to enable websockets'))
        .option('-W, --disable-web-socket', $('use this flag to disable websockets'))
        .option('-s, --subscription <id>', $('the subscription id'))
  ...
  site.doSitesPost = function (options, callback) {
    getHostNameSuffix(options.subscription, function (err, hostNameSuffix) {
      ...
      if (site.hostname) {
        site.HostNames['a:string'].push(site.hostname);
      }
      ...
      service.webSites.create(options.site.webspace, options.site.name, site, function (err, result) {
        progress.end();
        if (err) {
        ...
        if (err) {
          if (err.messagetemplate) {
            ...
            if (err.parameters && err.parameters['a:string']) {
              ...
              return callback(new Error(util.format.apply(util, errormessageargs)));
            }
  ...
  site.doRepositoryDelete = function(options, callback) {
    ...
    // TODO: this operation does not exist in the wrappers yet. Fix it.
  ...
  site.doAvailableLocationsGet = function (options, _) {
    ...
    try {
      ...
      var locations = service.webSpaces.listGeoRegions(_);
      ...
      var webspaces = service.webSpaces.list(_);
  ...
  site.doSpacesGet = function (options, _) {
    ...
    try {
      var service = createWebsiteManagementService(options.subscription);
      var spaces = service.webSpaces.list(_);
  ...
  site.doSitesGet = function (options, _) {
    ...
    if (!spaces) {
      ...
      try {
        spaces = service.webSpaces.list(_);
      } finally {
        progress.end();
      }
    }
    ...
    try {
      var result = async.map(spaces,
        function (webspace, _) {
          return service.webSpaces.listWebsites(webspace.Name, { propertiesToInclude: [
            'repositoryuri',
            'publishingpassword',
            'publishingusername'
          ] }, _);
  ...
  site.doSiteGet = function (options, callback) {
    ...
    // TODO: get web site seems to not have properties to include
    service.webSites.get(options.site.webspace, options.site.name, {
  ...
  site.doPublishingUsersGet = function (options, _) {
    ...
    try {
      try {
        var service = createWebsiteManagementService(options.subscription);
        var publishingUsers = service.webSpaces.listPublishingUsers(_);
  ...
  function getHostNameSuffix(subscription, callback) {
    ...
    if (subscriptionId) {
      ...
      websiteManagementService.getDnsSuffix(function (err, hostNameSuffix) {
        callback(err, hostNameSuffix.DnsSuffix || cli.environmentManager.getHostNameSuffix());
      });
    } else {
