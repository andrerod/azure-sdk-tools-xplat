...
exports.init = function (cli) {
  ...
  site.command('list')
        ...
        .execute(function (options, _) {
          ...
          // filter results per slot
          if (options.slot) {
            if (WebsitesClient.isProductionSlot(options.slot)) {
              sites = sites.filter(function (s) {
                return !utils.stringEndsWith(s.name, ')');
              });
            } else {
              sites = sites.filter(function (s) {
                return utils.stringEndsWith(s.name, util.format('(%s)', options.slot));
              });
            }
          }
  ...
  site.command('set [name]')
      ...
      var context = {
        subscription: cli.category('account').getCurrentSubscription(options.subscription).Id,
        site: { name: WebsitesClient.getSiteName(name, options.slot) }
      };
  ...
  site.command('create [name]')
        ...
        .execute(function (nameArg, options, _) {
          ...
          if (!context.flags.siteExists && options.slot) {
          ...
          function createSite(_) {
            ...
            if (!context.flags.siteExists) {
              ...
              var websiteAddress = context.site.name + '.' + getHostNameSuffix(context.subscription, _);
              ...
              websiteClient.createSite(context.subscription, context.site.webspace, site, _);
            }
          }
          ...
          function addRemoteToLocalGitRepo(_) {
            ...
            var gitUri = getGitUri(context.repo, WebsitesClient.getSiteName(context.site.name, context.site.slot), context.publishingUser);
  ...
  site.command('browse [name]')
        ...
        .execute(function (name, options, _) {
          ...
          var context = {
            subscription: cli.category('account').getCurrentSubscription(options.subscription).Id,
            site: {
              name: WebsitesClient.getSiteName(name, options.slot)
            }
          };
  ...
  site.command('delete [name]')
        ...
        .execute(function (name, options, _) {
          ...
          try {
            ...
            service.webSites.delete(context.site.webspace, context.site.name, {
                deleteMetrics: true,
  ...
  site.command('swap [name]')
        ...
        .execute(function (name, options, _) {
          ...
          if (!options.slot) {
            options.slot = WebsitesClient.getStagingSlotName();
          }

          lookupSiteNameAndWebSpace(context, _);

          if (!options.quiet && !confirm(util.format($('Swap slot %s from site %s into production '), options.slot, context.site.name), _)) {
            return;
          }

          var progress = cli.interaction.progress(util.format($('Swapping slot %s from site %s into production'), options.slot, context.site.name));
          try {
            var service = createWebsiteManagementService(context.subscription);
            service.webSites.swapSlots(context.site.webspace, context.site.name, options.slot, _);
          } finally {
            progress.end();
          }

          log.info(util.format($('Site %s slots has been swaped'), context.site.name));
        });
  ...
  site.doRepositoryPost = function (options, callback) {
    ...
    service.webSites.createRepository(options.site.webspace, options.site.name, function (err, result) {
        progress.end();
        if (err) {
          utils.logError(log, $('Failed to initialize repository'), err);
        } else {
  ...
  site.doRepositoryDelete = function(options, callback) {
    ...
    service.webSites.deleteRepository(options.site.webspace, options.site.name, function (err, result) {
        progress.end();
        if (err) {
          utils.logError(log, $('Failed to delete repository'), err);
        } else {
  ...
  site.doRepositorySync = function(options, callback) {
    ...
    service.webSites.syncRepository(options.site.webspace, options.site.name, function (err, result) {
        progress.end();
        if (err) {
          utils.logError(log, $('Failed to sync repository'), err);
        } else {
  ...
  site.doSiteGet = function (options, _) {
    ...
    try {
      ...
      utils.logError(log, $('Failed to get site info'), err);
  ...
  site.doSiteConfigGet = function (options, callback) {
    ...
    service.webSites.getConfiguration(options.site.webspace, options.site.name,
      function (err, result) {
        progress.end();
        if (err) {
          utils.logError(log, $('Failed to get site config info'), err);
        } else {
