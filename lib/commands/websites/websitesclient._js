...
__.extend(WebsitesClient.prototype, {
  ...
  addRemoteToLocalGitRepo: function (_) {
    var self = this;

    self.cli.output.silly('addRemoteToLocalGitRepo');
    if (!context.flags.isGitWorkingTree) {
      self.cli.output.info('To create a local git repository to publish to the remote site, please rerun this command with the --git flag: "azure site create ' + ((context.site && context.site.name) || '{site name}') + ' --git".');
      return;
    }
    ...
    self.cli.output.verbose($('Detecting git and local git folder'));
    ...
    if (azureExists) {
      self.cli.output.verbose($('Removing existing azure remote alias'));
    ...
    self.cli.output.info(util.format($('Executing `git remote add azure %s`'), gitUri));
