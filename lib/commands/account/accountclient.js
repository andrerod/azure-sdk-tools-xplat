...
_.extend(AccountClient.prototype, {
  ...
  importPublishSettings: function (file, options, callback) {
    ...
    if (keyPresent + certPresent === 1) {
      ...
      self.cli.output.verbose(util.format($('Key and cert have been written to %s'), pemPath));
    } else {
      ...
      try {
        ...
        self.cli.output.silly($('Unable to read file as xml publish settings file. Assuming it is pfx'));
    ...
    if (!options.skipregister && publishSettings && publishSettings.PublishProfile.Subscription) {
      var progress = self.cli.progress($('Verifying account'));
  ...
  registerKnownResourceTypes: function (subscriptionId, callback) {
    ...
    var service = utils.createServiceManagementService(
      subscriptionId, self, self.cli.output, REQUIRED_API_VERSION);
