...
exports.init = function (cli) {
  ...
  sbnamespace.command('create [namespace] [region]')
    ...
    .execute(function (namespaceName, region, options, _) {
      ...
      namespaceName = cli.interaction.promptIfNotGiven($('Namespace name: '), namespace, _);
      ...
      var createdNamespace = service.namespaces.create(namespaceName, region, _).namespace;
      progress.end();
  ...
  function createService(subscription) {
    return utils._createServiceBusClient(cli.category('account').getCurrentSubscription(subscription), log);
  }
};