...
function createService(serviceFactoryName, subscriptionId, account, logger) {
  ...
  var managementEndpoint = url.parse(getGlobalSettingsManager(account).getManagementEndpointUrl(account.managementEndpointUrl()));
...
function getGlobalSettingsManager(account) {
  var cli = account;
  while (cli.parent) {
    cli = cli.parent;
  }

  return cli.globalSettingsManager;
}
...
*/
exports.fileExists = function(path, cb) {
