...
Channel.prototype.send = function (method, data, cb, chunkcb) {
  ...
  var req = https.request(options, function (res) {
    ...
    function processResponse (err, content) {
      // for fail statusCode
      if (!err && res.statusCode && res.statusCode >= 400) {
        var errorCode = (content && content.Code) || res.statusCode;

        var ret = new Error((content && content.Message) || content || 'fail with ' + errorCode + ' statusCode');
        ret.Code = errorCode;
    ...
    res.on('end', function () {
      if (!dataLen) {
        return processResponse(null, null);
      }
      ...
      parseContent(contentType, buf, processResponse);
    });
  }).on('error', function (e) {
